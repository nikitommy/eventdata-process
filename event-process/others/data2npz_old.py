...
from reblur2 import basename



def data2npz(opt,events):
    pic_dir_names = sorted(glob.glob(os.path.join(opt.pic_path, '*')))
    picgray_dir_names = sorted(glob.glob(os.path.join(opt.picgray_path, '*')))

    events, exp_start1, exp_end1, exp_start2, exp_end2 = event2npy(opt,events)

    pic1 = cv2.imread(pic_dir_names[x*2])
    blur1 = color2gray(pic1).astype(np.float64)
    #blur1 = blur1.astype(np.float64)
    save_pic(blur1, opt.picgray_path, pic_dir_names[x*2])
    
    pic2 = cv2.imread(pic_dir_names[x*2+1])
    blur2 = color2gray(pic2).astype(np.float64)
    #blur2 = blur2.astype(np.float64)
    save_pic(blur2, opt.picgray_path, pic_dir_names[x*2+1])
    ...
    filename = basename(pic_dir_names[x]) + '-' + basename(pic_dir_names[x+1]) + '.npz'
...
if __name__ == '__main__':
    ...
    pic_dir_names = sorted(glob.glob(os.path.join(opt.pic_path, '*')))
    for i in range(len(pic_dir_names)-1):
        data2npz(opt,i)
